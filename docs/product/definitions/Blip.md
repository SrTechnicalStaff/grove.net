...
> **What is a blip?**
> A **Blip** is the passive, unengaged visual indicator of an available Annotation on the Grid. It manifests as a compact, pulsating indicator dot at the center of dense Aura field concentrations, signaling synthesized multi-layer knowledge without cluttering the spatial canvas.

---

## 1. Core Essence ("What is a blip?")

- **Canonical Statement**: A Blip is a dormant Annotation trigger. It marks the focal point of a high-density Aura cluster on the Information Layer, maintaining visual quietness until cursor proximity escalates it into an active Annotation overlay.
- **Primary Function**: Blips prevent visual noise across large workspaces. They compress complex multi-layer content clusters into minimal ambient signals, letting users scan distant spatial regions without opening popups prematurely.
- **Mental / Physical Model**: 
  ...
  2. *Navigational Buoy / Beacon*: Acts as a spatial beacon anchored to gravitational centers in the workspace.

---

## 2. Fundamental Invariants & System Properties

1. **Passive Existence**: A Blip exists in a passive state by default. It requires zero user interaction to maintain its visual beacon on the Information Layer.
...
### Data & State Schema
- **State Ownership**: Information Layer Subsystem (`BlipManager`).
...
| :--- | :--- |
| **Grid** | Blips anchor to precise Grid cell coordinates, rendering as floating indicators above the 2D plane on the Information Layer. |
...
| **Aura** | Blips generate at points of peak Aura field overlap and intensity within the Field Ledger across all stacked Layers. |
| **Layer** | Blips aggregate Aura energy vertically across **all** stacked Layers, signaling multi-layer activity even on inactive Z-planes. |
...
| **Slate** | Opening a Blip's Annotation in Slate dismisses the spatial Blip overlay in favor of a fixed-viewport reading window. |
...
- **Camera Zoom / LOD Behavior**:
  ...
  - At distance / macro zoom, Blips become the primary visual indicators for navigating large clusters across the workspace.
...
- **[Notice Annotation Markers](../derived-outcomes/Uncluttered%20Visual%20Scanning%20At%20Distance.md)**: Users spot pulsating ambient signals indicating rich context clusters across their workspace.
