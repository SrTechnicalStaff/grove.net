...
| **`05-presence-fields.html`** | Cell-quantized aura, lit cells with hard boundaries, discrete alpha tiers (`fw1` 0.19 to `fw4` 0.038, `fh1` 0.60 to `fh4` 0.11, `fi1` 0.30 to `fi4` 0.05), stepped falloff, perimeter izolines ($E \ge 0.15$), derived non-persisted state, cross-layer hue summation (~100 layers as atmosphere), local recompute during drag with 1-frame aura lag trade-off (`PA-02`), refusal of smooth gradient glow, ghost content from non-active layers, and field brighter than content. | `ADR-002`, `ADR-003` | Discrete alpha tier breakdown for neutral/warm/indigo modes, hard cell edge boundary rule (no smooth gradient blurs across cell boundaries), 1-frame aura lag trade-off contract during drag gestures, smooth glow refusal, ghost content from inactive layers refusal, field over content luminance refusal. |
...
- **Surgical Addition**: Appended Section 6.1 (Discrete Alpha Tiers for neutral `--fw` 0.19–0.038, warm `--fh` 0.60–0.11, anchor indigo `--fi` 0.30–0.05; hard cell edge boundaries), Section 6.2 (Gesture Latency Trade-off: 0ms pointer tracking, local recompute, 1-frame aura lag `PA-02`, origin ghost), and Section 6.3 (Presence Field Refusal Contracts: smooth glow refusal, ghost content from inactive layers refusal, field over content luminance refusal).
