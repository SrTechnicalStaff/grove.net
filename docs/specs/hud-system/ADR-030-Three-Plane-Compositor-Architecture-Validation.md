...
2. **Plane 1 (Information Plane)**: World-anchored editorial surfaces (local text editors, inline annotation markers, image viewports) unscaled by camera zoom.
3. **Plane 2 (HUD Overlay Plane)**: Viewport-fixed application chrome, composed Writing Slate, Memory Slate, Gallery Slate, the Layer Manager overlay, and operation controls.
...
- **Zero Level-Lifting**: Plane 0 NEVER mutates its z-index or rendering order to obscure or compete with Plane 1 or Plane 2.
...
```
                      ...
                      |                                                             |
                      |  +-------------------------------------------------------+  |
                      |  | Plane 2: HUD OverlayLayer (ZIndex = 400)             |  |
                      |  | - Named Slates (Memory, Gallery, Writing)           |  |
                      |  | - Layer Manager overlay                          |  |
                      |  +-------------------------------------------------------+  |
                      |                              |                              |
                      |                              v Pass-Through Input           |
                      |  +-------------------------------------------------------+  |
                      |  | Plane 1: Information Canvas (ZIndex = 300)          |  |
                      |  | - Local Text Editors, Annotation Overlays             |  |
                      |  +-------------------------------------------------------+  |
                      |                              |                              |
                      |                              v Pass-Through Input           |
                      |  +-------------------------------------------------------+  |
                      |  | Plane 0: Custom Skia SKCanvas Control (ZIndex = 10)   |  |
...
*Description*: Render all 3 planes inside a single custom Avalonia `Control` executing custom `SKCanvas` draw calls for every element (grid, text editors, named Slates, overlays, controls).
...
*Description*: Host Plane 0 on the main window, and spawn separate borderless native OS windows (`Avalonia.Controls.Window`) for Plane 1 overlays and the Plane 2 named-Slate surfaces.
...
| :--- | :--- | :--- | :--- | :--- |
| **Plane 0: Spatial Grid Canvas** | `GridCanvasControl : Control` | `ZIndex = 10` | Camera Matrix $T(x,y,s)$ World Space | Consumes pointer when clicking placed items, selecting cells, or dragging/panning grid. |
| **Plane 1: Information Plane** | `InformationPlaneCanvas : Canvas` | `ZIndex = 300` | Screen Space $(x_s, y_s)$ from World Origin | Hits active local editor bounds only; transparent outside editor frames. |
| **Plane 2: HUD Overlay Plane** | `HudOverlayPanel : Panel` | `ZIndex = 400` | Absolute Viewport Pixels $[0, W] \times [0, H]$ | Hits active named-Slate frames and overlays; transparent pass-through outside their bounds. |
...
[Avalonia TopLevel Window Input Pipeline]
         │
         ▼
[Plane 2: HUD Plane Check (ZIndex = 400)]
   ├── Hit named-Slate frame / overlay control? ──► YES ──► Consume Event in HUD surface
   └── NO (Hit Transparent Viewport Region)
         │
         ▼
[Plane 1: Information Plane Check (ZIndex = 300)]
   ├── Hit Local Editor / Annotation? ──► YES ──► Consume Event in Local Editor
   └── NO (Hit Transparent Canvas Region)
         │
         ▼
[Plane 0: Spatial Canvas Check (ZIndex = 10)]
   └── Handle Camera Pan, Zoom, Cell Selection, or Content Interaction
```

### Hit-Testing & Pass-Through Mechanics
1. **Pass-Through Container Rules**: `HudOverlayPanel` (Plane 2) and `InformationPlaneCanvas` (Plane 1) set `Background="Transparent"` and override Avalonia hit-test behavior. Unbound pointer coordinates return `false` on hit tests, enabling pointer events to fall through to lower planes seamlessly without visual dimming or modal blocking.
...
| **Plane 2 Named-Slate Open / Render Latency** | $< 16.0\text{ ms}$ (1 frame) | **4.2 ms** | **PASSED** |
...
{
    Plane0_SpatialGridCanvas = 10,
    Plane1_InformationPlane = 300,
    Plane2_HudSlatePlane = 400
}
...
public sealed class ThreePlaneCompositorHost : Panel
{
    ...
    public ThreePlaneCompositorHost()
    {
        ...
        _plane1InformationCanvas = new Canvas { ZIndex = (int)VisualPlaneOrder.Plane1_InformationPlane, Background = Brushes.Transparent };
